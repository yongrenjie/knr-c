...
int pushback = 0; // 0 if there is no character pushed back, 1 if there is
char buf = 0; // stores the pushed-back character

int getch(void) /* get a (possibly pushed back) character */
{
    int temp = pushback;
    pushback = 0; // doesn't matter if this is 0 or 1 before; we want it to be 0 regardless
    return (temp) ? buf : getchar();
}
